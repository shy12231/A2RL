...
def Sampling_RL(actor, critic, x, vector, aspect_idx, Random=True, epsilon=0.1):
    ...
    start, end = aspect_idx[0], aspect_idx[1]
    aspect_embed = vector[start: end+1]
    ...
    for pos in range(len(vector)):
        predicted = actor.get_target_output(current_lower_state, vector[pos], scope = "target")
        states.append([current_lower_state, vector[pos]])
        if Random:
...
class actor(nn.Module):
    ...
    def get_gradient(self, h, x, reward, scope):
        if scope == "target":
            out = self.target_policy(h, x) # torch.Size([2, 1])
